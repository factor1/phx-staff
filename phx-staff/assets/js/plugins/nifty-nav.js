...
jQuery( document ).ready( function( $ ) {
  $( document ).ready( function() {
    ...
    $( '#nifty-nav-toggle' ).click( function() { //listen for the click on the menu icon
      i++; // Add 1 to the count
      $( '#nifty-nav-toggle' ).toggleClass( 'active' );
      $( '.nifty-panel' ).slideToggle( 500 ).css( "position",
        "relative" ); //toggle the nav up/down
      ...
      $( '.mask' ).click( function() { //listen for the click on the mask
        $( '.nifty-panel' ).slideUp( 500 ); //pull up the nav
        maskit();
        $( '#nifty-nav-toggle' ).removeClass( 'active' ); //mmm hamburger
        i++; // Add 1 to the count
      } );
      ...
      $( '.nav-item' ).click( function() { //oh snap, you want to close the nav on a link click?
        $( '.nifty-panel' ).slideUp( 500 ); //pull up the nav
        maskit();
        $( '#nifty-nav-toggle' ).removeClass( 'active' );
